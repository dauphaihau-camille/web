...
export function DocumentBreadcrumb({
  ...
  const [breadcrumbDocuments, setBreadcrumbDocuments] = useState<Document[]>([document]);
  ...
  useEffect(() => {
    ...
    async function loadBreadcrumbDocuments() {
      ...
      while (parentDocumentId && !visitedDocumentIds.has(parentDocumentId)) {
        ...
        const cachedParentDocument =
          queryClient.getQueryData<Document>(documentKeys.detail(parentDocumentId));
        const parentDocument = cachedParentDocument ??
          await getDocument(parentDocumentId);
    ...
    >
      <BreadcrumbList className="min-w-0 flex-nowrap gap-0.5 text-sm">
        {visibleBreadcrumbDocuments.map((breadcrumbDocument, index) => {
          ...
          const showEllipsis = resolvedBreadcrumbDocuments.length > 3 && index === 1;
