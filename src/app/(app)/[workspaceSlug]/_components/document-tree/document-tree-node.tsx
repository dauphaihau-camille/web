...
import { parseDocumentRouteSegment, workspaceRoutes } from '@/domains/workspace';
...
export function DocumentTreeNode({
  ...
  const href = workspaceRoutes.document(workspaceSlug, document.public_id, document.title);
  ...
  const isExpanded = expandedByWorkspace[workspaceSlug]?.includes(document.id) ?? false;

  const toggleExpandedDocumentId = useDocumentTreeExpansionStore(
    (state) => state.toggleExpandedDocumentId,
  );

  const childrenQuery = useWorkspaceDocumentChildrenQuery(workspaceSlug, document.id, {
