...
export function DocumentTreeNodeActions({
  ...
  const createSubdocumentMutation = useMutation({
    ...
    onSuccess: async (childDocument) => {
      ...
      markCachedNavigationNodeHasChildren(queryClient, workspaceSlug, document.id);
      insertCreatedSubdocIntoCachedChildren(
        queryClient,
        workspaceSlug,
        document.id,
      ...
      <DropdownMenu onOpenChange={setIsMenuOpen}>
        ...
        <DropdownMenuContent side="right" sideOffset={8} className="w-auto min-w-48">
          ...
          >
            ...
            <span>{duplicateDocumentMutation.isPending ? 'Duplicating...' : 'Duplicate'}</span>
          </DropdownMenuItem>
          ...
          <DropdownMenuItem
            disabled={favoriteMutation.isPending || favoriteStatusQuery.isLoading}
            onClick={handleToggleFavorite}
          >
            <StarIcon className={cn('size-4', isFavorite && 'fill-current text-amber-300')} />
          ...
          >
            ...
            <span>{archiveDocumentMutation.isPending ? 'Moving to Trash...' : 'Move to Trash'}</span>
          </DropdownMenuItem>
        </DropdownMenuContent>
      </DropdownMenu>
    </div>
