import WorkspaceNamespaceEntryPage from './page';
...
describe('WorkspaceNamespaceEntryPage', () => {
  beforeEach(() => {
  ...
  it('redirects directly to the default workspace document', async () => {
    ...
    await expect(WorkspaceNamespaceEntryPage()).rejects.toMatchObject({
      path: '/w/acme-product/untitled-91f35adf4eb9b9b15ec4c8a97cb1f912',
    });

    expect(requireCurrentUserServerMock).toHaveBeenCalledWith('/w');
  ...
  it('falls back to the workspace entry when there is no default workspace', async () => {
    ...
    expect(getWorkspaceDefaultDocumentServerMock).not.toHaveBeenCalled();
